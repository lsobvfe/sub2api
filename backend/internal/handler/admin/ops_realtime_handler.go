...
// GetActiveStreamHolds returns streaming Responses requests currently retained
// before the first semantic downstream output.
// GET /api/v1/admin/ops/stream-holds
func (h *OpsHandler) GetActiveStreamHolds(c *gin.Context) {
	if h.opsService == nil {
		response.Error(c, http.StatusServiceUnavailable, "Ops service not available")
		return
	}
	if h.streamHoldTracker == nil {
		response.Error(c, http.StatusServiceUnavailable, "OpenAI stream hold tracker not available")
		return
	}
	if err := h.opsService.RequireMonitoringEnabled(c.Request.Context()); err != nil {
		response.ErrorFrom(c, err)
	...
	snapshot, err := h.streamHoldTracker.List(c.Request.Context(), filter)
	if err != nil {
		if isOpsRealtimeRequestCanceled(c, err) {
			return
		}
