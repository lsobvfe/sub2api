...
type APIKey struct {
	...
	// Whether this API key uses the local stream hold proxy
	StreamHoldEnabled bool `json:"stream_hold_enabled,omitempty"`
...
func (*APIKey) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case apikey.FieldIPWhitelist, apikey.FieldIPBlacklist:
			values[i] = new([]byte)
		case apikey.FieldStreamHoldEnabled:
	...
	for i := range columns {
		...
		case apikey.FieldStreamHoldEnabled:
			if value, ok := values[i].(*sql.NullBool); !ok {
				return fmt.Errorf("unexpected type %T for field stream_hold_enabled", values[i])
			} else if value.Valid {
				_m.StreamHoldEnabled = value.Bool
			}
...
func (_m *APIKey) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("stream_hold_enabled=")
	builder.WriteString(fmt.Sprintf("%v", _m.StreamHoldEnabled))
	builder.WriteString(", ")
